...
# --- User-specified MongoDB URI ---
# NOTE: Using the hardcoded URI as requested by the user.
HARDCODED_MONGODB_URI = "mongodb+srv://room:room@room.4vris.mongodb.net/?appName=room"

def init_mongodb():
    global client, db, content_collection, counter_collection
    
    try:
        # Prioritize the user-provided hardcoded URI, fall back to environment variable
        MONGODB_URI = HARDCODED_MONGODB_URI or os.environ.get("MONGODB_URI")
        ...
        # Using 'diskwala_posts' as the collection name to accommodate user's request
        # to isolate diskwala data for auto-repost, while keeping it as the main content store.
        ...
        logger.info(f"MongoDB connected successfully. Database: {db_name}, Collection: {collection_name}")
        return True
...
def flush_view_cache():
    ...
    while True:
        # Reduced sleep time for faster testing/data persistence
        time.sleep(10)
        try:
            with cache_lock:
                ...
                bulk_ops = []
                # Use a dictionary comprehension to ensure a copy of keys is processed
                # while allowing updates to the cache in the next iteration.
                keys_to_process = list(view_count_cache.keys())
                
                for cache_key in keys_to_process:
                    count = view_count_cache.get(cache_key, 0)
                    if count > 0:
                        ...
                        if ObjectId.is_valid(content_id):
                            ...
                            # Clear processed entries from the cache
                            del view_count_cache[cache_key]
